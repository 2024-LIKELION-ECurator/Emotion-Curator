...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://13.125.7.131/users/register/', {
        username: formData.username,
        password: formData.password,
        nickname: formData.nickname,
        birthdate: `${formData.birthYear}-${formData.birthMonth}-${formData.birthDay}`, // 'birth' -> 'birthdate'로 수정
      });
